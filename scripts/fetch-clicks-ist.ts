...
async function run() {
  ...
  const startDate = new Date(`${targetDateStr}T00:00:00.000+05:30`);
  const endDate = new Date(`${targetDateStr}T23:59:59.999+05:30`);
  ...
  const outputPath = path.resolve(`clicks_${targetDateStr}_IST.xlsx`);
